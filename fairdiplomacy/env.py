...
global global_prev_orders
global_prev_orders = None
...
class OneSixPolicyProfile(BasePolicyProfile):
    ...
    def __init__(
        ...
        self._players[agent_one_power + '_build'] = Player(agent_six, agent_one_power)
        self.share_strategy = share_strategy

    def get_player(self, power: Power) -> Player[BaseAgent]:
        return self._players[power]

    def get_agent_one_power(self):
        return self._agent_one_power
    ...
    def get_all_power_orders(self, game) -> JointAction:
        ...
        if 'MOVEMENT' != phase_ours[-1]:
            ...
            invalid_orders = None
            if 'invalid' in orders[self._agent_one_power]: #部分替换    
                invalid_orders = orders[self._agent_one_power]       
                if len(orders[self._agent_one_power]) == 1:
                    orders[self._agent_one_power] = self._players[self._agent_one_power + '_build'].get_orders(game_from_view_of(game, self._agent_one_power))
                    logging.info("Invalid orders, using base strategy replacement")
                else:
                    orders[self._agent_one_power] =  orders[self._agent_one_power][:-1]   
                with open(order_path, 'a') as file:
                    file.write(f"invalid orders:\n{invalid_orders}\nblueprint orders:\n{self._players[self._agent_one_power + '_build'].get_orders(game_from_view_of(game, self._agent_one_power))}\n\n")               
            # if 'invalid' in orders[self._agent_one_power]: #完全替换
            #         orders[self._agent_one_power] = self._players[self._agent_one_power + '_build'].get_orders(game_from_view_of(game, self._agent_one_power))
...
class Env:
    def __init__(
        ...
        *,
        ...
        self.global_prev_orders_path = f'global_prev_order/{self.policy_profile._agent_one_power}/{game.game_id}.json'
        if not os.path.exists(self.global_prev_orders_path):
            os.makedirs(os.path.dirname(self.global_prev_orders_path), exist_ok=True)
        with open(self.global_prev_orders_path, 'w') as file:
            json.dump({}, file)  # 写入一个空的 JSON 对象，可以根据需要更改内容
    ...
    def process_turn(self, timeout=10):
        ...
        phase_ours = self.game.phase.split(' ')

        if "MOVEMENT" == phase_ours[-1]:
            if not os.path.exists(self.global_prev_orders_path):
                os.makedirs(os.path.dirname(self.global_prev_orders_path), exist_ok=True)
            with open(self.global_prev_orders_path, 'w') as file:
                json.dump(power_orders, file)

        logging.info("Finished order prediction for turn {}".format(self.game.phase))
        # logging.info("power_orders {}".format(power_orders))
