...
class Env:
    ...
    def process_turn(self, timeout=10):
        ...
        global_prev_orders_path = 'global_prev_orders.json'
        directory = os.path.dirname(global_prev_orders_path)
        if not os.path.exists(directory):
            os.makedirs(directory)  

        if "ADJUSTMENTS" == phase_ours[-1]:
            with open(global_prev_orders_path, 'w') as file:
