...
def run_1v6_trial(
    ...
    html_file = game_to_html(env.game, "test.html")
...
def run_1v6_trial_multigame(
    agent_one, agent_six, agent_one_power: str, cfg: conf_cfgs.CompareAgentsTask, game_seed: int, cf_agent=None
):
    """Run a trial of 1x agent_one vs. 6x agent_six
    ...
    if cfg.out is not None:
        # cfg.out = f"{agent_one_power}_{game_seed}_{cfg.out}"
        cfg_path = f"{agent_one_power}/{game_seed}_{cfg.out}"
        pathlib.Path(cfg_path).parent.mkdir(exist_ok=True, parents=True)

    partial_out_name = cfg.out + ".partial" if cfg.out else None
    partial_out_name = f"{agent_one_power}_{game_seed}_{partial_out_name}"
    annotations_out_name = (
        pathlib.Path(cfg.out.rsplit(".", 1)[0] + ".metann.jsonl") if cfg.out else None
    )
    with maybe_kickoff_annotations(env.game, annotations_out_name):
    ...
    if all(s < 18 for s in scores.values()):
        ...
        winning_power = max(scores, key=lambda x: scores[x])
        result = "one" if winning_power == agent_one_power else "six"
    game_info =  f"Scores: {scores}, Winner: {winning_power}, agent_one_power= {agent_one_power}"
    logging.info(game_info)

    html_file = game_to_html(env.game, "test.html")

    with open(f"{pathlib.Path(cfg_path).parent}/{game_seed}.html", "w") as f:
        f.write(html_file)
