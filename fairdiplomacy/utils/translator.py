INPUT_FILE_PATH = "diplomacy_experiments/dataset_with_html/high_dataset_0721/c04_exploit/research_20240409_collect_expert_data/lau.loc.use@true_b2bca929/buffer1.bin"
OUTPUT_FILE_PATH = "diplomacy_experiments/Prompt/high_0721/prompt_policy_5act.json"
...
def create_user_prompt_hier(board_state_content, country):
    regions = board_state_content.strip().split("\n\n")
    occupation_stats = defaultdict(lambda: defaultdict(lambda: {'names': [], 'count': 0, 'attributes': []}))
    first_region = True
    for region in regions:
        ...
        for line in region_info:
            ...
            if "this area is" in line and len(line.split()) == 4 and "coast" not in line:
                ...
                if "home center" in line and "neither" not in line:
                    attributes.append(line[13:])
                elif "this area is a supply center, currently occupied by" in line:
                    index = line.find("occupied by")
                    occupied_power= line[index+len("occupied by"):].strip()
                    attributes.append(f"supply center occupied by {occupied_power}") 
    ...
    sorted_items = sorted(occupation_stats.items(), key=lambda item: item[0])
    for power, regions in sorted_items:
        user_prompt += f"{power.capitalize()}:"
        sorted_regions = sorted(regions.items(), key=lambda item: item[0])
        for region_name, stats in sorted_regions:
            # 假设 attributes 是一个字符串，如果不是，需要相应地转换
            if len(stats['attributes']) == 0:
...
def orders2str(orders, possible_action):
    ...
    if len(orders_str) == 0:
        orders_str = "all army and fleet hold\n"
...
def country_orders2str(country_search_policy_order, country_search_policy_prob, country_possible_action, num_actions=1):
    ...
    for idx in largest_prob_indices:
        combined_order_str += f"[ACTION {count}]\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--playing_countries", nargs="*", default=[],
                        help="List all countries, for example, --playing_countries A B, if its empty or not provided, will default to all countries.")
    ...
    for content_dict in tqdm.tqdm(contents, desc="decoding dataset"):
        ...
        for transition_id in range(board_states.size(0)):
            ...
            for country_index in range(7):
                ...
                order_possible_action = torch.ones(possible_action[country_index, :].shape)

                country_search_policy_prob = search_policy_prob[country_index, :]
                country_search_policy_order = search_policy_order[country_index, :]

                order_description = country_orders2str(country_search_policy_order, country_search_policy_prob,
                                                       country_possible_action, num_actions=args.num_actions)
                ...
                system_content = "You are an expert in the no-press Diplomacy environment. Your task is to play as one of the seven powers, using your army or fleet to control the centers on the board to win the game. You are observing the [CURRENT STATE], which includes the [Game Time and Phase] and [Board State] sections below. In the [Board State], each power will sequentially display its occupied areas, each with the format Area (attribute 1, attribute 2,...). Remember, the default attributes for areas—(coast, neither supply center nor home center, troops non-buildable here, troops non-removable here, no troops dislodged here)—will be omitted unless specified otherwise. Your response should be in the following format:\n[ACTION ]\n- \n- \n...\n- "
                user_content = create_user_prompt(description, country_name)
