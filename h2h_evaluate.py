...
from fairdiplomacy.compare_agents import run_1v6_trial_multigame, run_1v6_trial_multiprocess
...
def compare_agents(cfg):
    ...
    for game in range(GAMES):
        if cfg.seed >= 0:
            game_seed = game + cfg.seed * 1000
            logging.info(f"Set seed to {game_seed}")
        ...
        for power_string in power_strings:
            if cfg.num_processes > 0:
                ...
                result, game_info = run_1v6_trial_multigame(agent_one, agent_six, power_string, cfg, game_seed, cf_agent=cf_agent)
                # logging.warning("Result: {}".format(result))
                logging.warning(f"POWER: {power_string}, GAME: {game}, Result:{result}")    
                with open(f"results.info", "a") as f:
                    f.write(f"POWER: {power_string}, SEED: {game_seed}, {game_info}\n")
